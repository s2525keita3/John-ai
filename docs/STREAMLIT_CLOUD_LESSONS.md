...
| 訪問件数だけ追加が要る場合 | Cloud の **Advanced settings → Requirements file** に **`requirements_homon_kensu.txt`** を指定（未指定だとルート `requirements.txt` のみが使われ **pdfplumber が入らず**、集計が古い／空に見えることがある） |
