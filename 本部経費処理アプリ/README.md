...
- **店舗モード（桜木町・新子安・白根・さいわい）:** 各店舗のあおぞらCSVをドロップ→店舗別にスタッフ給与・資金移動・ATMを除外→マスタで自動分類し、**判断不能はレビュー表＋社長向けCSV＋クイックマスタ登録**に分離
- **店舗科目レイヤー（v1.3.0）:** 同じ入力から **経費Excel／収支計画スプシと同じ科目** で仕分け（給与・社保も除外せず科目付け）。本部管理費明細で月末の一括振込を分解、小口の「科目」列を尊重、**経費Excel月次シート（xlsx）とPL行＋セルメモ（CSV）** を出力
...
| 2026-07 | 店舗モードを桜木町から **新子安・白根・さいわい** へ横展開（v1.2.0）。店舗別給与除外・CSV取り違え警告・家賃系マスタ整備。2026年1〜7月の実CSV3店舗分で検証 |
| 2026-09 | **店舗科目レイヤー**（v1.3.0）。経費Excel／収支タブと同じ科目で仕分け・本部管理費明細で一括振込を分解・小口の科目列対応・経費Excel月次シートxlsx出力（桜木町マスタ約220行、バックテスト98〜100%） |
